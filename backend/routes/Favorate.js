        ...
        .then(exit => {
            if (!exit) {
                Favorate.create(FavorateCreate)
                    .then(() =>
                        res.json({ status: "favorate created" }))
                    .catch(err => {
                        res.send('error: ' + err)
                    })
            } else {
                res.json({ error: 'Favorate already exit' })
            }
        })
})
...
router.delete("/delete/:cid/:rid",function(req, res, next){
    ...
    .then(()=>{
        res.json({status:"the favorate will not longer exit, but you can get it back by create in again"})
    })
    .catch((error)=>{
